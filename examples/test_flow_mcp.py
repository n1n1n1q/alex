...
async def demonstrate_flow():
    ...
    server_params = StdioServerParameters(
        command=sys.executable, args=[cfg.mcp_server_path], env=None
    )
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            result = await session.call_tool(
                "plan_actions", arguments={"game_state": game_state}
            )
            ...
            messages = [
                {
                    "role": "system",
                    "content": "You are an expert Minecraft agent planner. You must output ONLY valid JSON. Do not include markdown formatting or explanations.",
                },
            ...
            validation_result = await session.call_tool(
                "validate_action_plan", arguments={"plan_json": plan_json}
            )
