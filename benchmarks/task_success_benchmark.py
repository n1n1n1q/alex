...
class TaskSuccessBenchmarkCallback(MinecraftCallback):
    ...
    def after_step(self, sim, obs, reward, terminated, truncated, info):
        ...
        if not self.task_completed and self.checker(info):
            ...
            print(
                f"[BENCHMARK] Task '{self.task_name}' completed at step {self.current_step}!"
            )

        if self.current_step % 50 == 0:
            print(
                f"[BENCHMARK] Step {self.current_step}/{self.max_steps}, Completed: {self.task_completed}"
            )
    ...
    def get_trial_result(self) -> Dict[str, Any]:
        return {
            ...
            "final_progress": (
                self.progress_log[-1]["progress"] if self.progress_log else 0.0
            ),
        }
...
class STEVEBenchmarkCallback(TaskSuccessBenchmarkCallback):
    ...
    def __init__(
        self, task_name: str, policy, max_steps: int = 6000, update_interval: int = 100
    ):
        super().__init__(task_name, max_steps)
        self.policy = policy
...
class ALEXBenchmarkCallback(TaskSuccessBenchmarkCallback):
    ...
    def __init__(
        self, task_name: str, max_steps: int = 6000, update_interval: int = 100
    ):
        super().__init__(task_name, max_steps)
...
def run_task_benchmark(
    ...
    try:
        ...
        print(f"{'='*80}")
        print(
            f"Success Rate: {stats.get('success_rate', 0):.1%} ({stats.get('num_successes', 0)}/{num_trials})"
        )
        if stats.get("avg_completion_time"):
            print(
                f"Avg Completion Time: {stats['avg_completion_time']:.2f}s ± {stats['std_completion_time']:.2f}s"
            )
...
if __name__ == "__main__":
    ...
    trials = (
        args.trials if args.trials else config.get_task_trials(args.task, args.model)
    )
    max_steps = (
        args.max_steps
        if args.max_steps
        else config.get_task_max_steps(args.task, args.model)
    )
