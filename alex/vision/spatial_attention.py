...
class SpatialAttentionMap:
    ...
    def get_patch_embeddings(self, image_tensor: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            x = torch.cat([vision_model.cls_token.repeat((B, 1, 1)), x], dim=1)
    ...
    def compute_similarity_maps(
        ...
        if use_softmax:
            similarity_maps = F.softmax(similarity_maps / temperature, dim=1)
    ...
    def downsample_to_4x4(self, similarity_map: torch.Tensor) -> torch.Tensor:
        ...
        depth_ranges = [(0, 3), (3, 6), (6, 8), (8, 10)]

        horiz_ranges = [(0, 4), (4, 8), (8, 12), (12, 16)]
    ...
    def _extract_detections(
        self, grids_4x4: Dict[str, torch.Tensor], threshold: float
    ) -> list[dict]:
        """
