    ...
    def analyze_frame(
        ...
        if global_queries:
            with torch.no_grad():
                ...
                global_text_norm = torch.nn.functional.normalize(global_text_embeddings, dim=-1)
...
def analyze_image(spatial_system: SpatialAttentionMap, image_path: str, text_queries: List[str], 
                  use_softmax: bool = True, global_queries: List[str] = None):
    """
    ...
    if result.get("global_scores"):
        ...
        print("-" * 80)
        sorted_globals = sorted(result["global_scores"].items(), key=lambda x: x[1], reverse=True)
    ...
    if "grids_4x4" in result:
        ...
        for query, grid in result["grids_4x4"].items():
            ...
            max_row = max_idx // 4
            max_col = max_idx % 4
            detections.append((max_val, query, depth_labels[max_row], horiz_labels[max_col]))
...
def main():
    ...
    if not images_dir.exists():
        ...
        text_queries = ["a creeper", "iron ore", "wooden planks", "stone blocks", "grass", "trees"]
        result = spatial_system.analyze_frame(dummy_image, text_queries, threshold=0.25, return_grids=True)
    ...
    queries_by_scene = {
        "zombie": {
            ...
            "global": ["hostile environment", "night time", "dangerous situation", "combat", "dark"]
        },
        "nether": {
            "spatial": ["lava", "netherrack", "ghast", "fire", "fortress"],
            "global": ["nether dimension", "hell biome", "red landscape", "dangerous", "hot environment"]
        },
        "taiga": {
            "spatial": ["spruce trees", "snow patches", "ice", "animals"],
            "global": ["snowy biome", "cold environment", "winter forest", "taiga", "peaceful"]
        },
        "chicken": {
        ...
        "indoor": {
            ...
            "global": ["enclosed space", "building interior", "shelter", "protected area", "base"]
        },
        "default": {
            "spatial": ["trees", "grass", "stone", "wood", "animals", "mobs"],
            "global": ["overworld", "natural terrain", "wilderness", "outdoor", "minecraft world"]
        }
    }
