    ...
    with open(report_file, "w") as f:
        ...
        for task in tasks:
            ...
            for model in results["models"]:
                ...
                if "error" in task_data:
                    ...
                    f.write(
                        f"  {model.upper():8s}: {success_rate:5.1f}% ({num_successes}/{num_trials})"
                    )
        ...
        for model in results["models"]:
            ...
            if "error" in dirt_data:
                ...
                f.write(
                    f"{model.upper():<10s} {best:<10d} {avg:6.1f} ± {std:4.1f}    {rate:5.2f} ± {rate_std:4.2f}\n"
                )
            else:
...
def main():
    ...
    parser.add_argument(
        "--steve-model", type=str, help="Path to STEVE checkpoint (also used by ALEX)"
    )

    parser.add_argument("--task-trials", type=int, default=10, help="Trials per task")
    parser.add_argument(
        "--task-max-steps", type=int, default=6000, help="Max steps per task trial"
    )
    parser.add_argument("--dirt-trials", type=int, default=5, help="Dirt mining trials")
    parser.add_argument(
        "--dirt-max-steps", type=int, default=3000, help="Max steps per dirt trial"
    )
    ...
    if "steve" in args.models:
        if not args.steve_model:
            ...
            return
        models_config["steve"] = {"model_path": args.steve_model}

    if "alex" in args.models:
        if not args.steve_model:
            print(
                "ERROR: --steve-model required for ALEX benchmark (ALEX uses STEVE-1 as backbone)"
            )
            return
        models_config["alex"] = {"model_path": args.steve_model}
