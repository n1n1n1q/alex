...
class HuggingFaceReflexManager:
    ...
    def __init__(
        ...
        self.device = (
            device or cfg.device or ("cuda" if torch.cuda.is_available() else "cpu")
        )
        self.temperature = (
            temperature if temperature is not None else cfg.hf_reflex_temperature
        )
        self.max_new_tokens = (
            max_new_tokens if max_new_tokens is not None else cfg.hf_reflex_max_tokens
        )

        if self.verbose:
            print(
                f"[HuggingFaceReflexManager] Loading model: {self.model_name} on {self.device}..."
            )
    ...
    def detect(self, state: GameState) -> Optional[Subgoal]:
        ...
        try:
            ...
            priority = int(
                parsed.get(
                    "priority", self.allowed_actions.get(action, {}).get("priority", 50)
                )
            )

            if self.verbose:
                print(
                    f"[ReflexManager] Model action: {action}, priority={priority}, params={params}"
                )
    ...
    def _summarize_state(self, state: GameState) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "hunger": state.hunger,
            "has_shield": any(
                slot.get("item_id") == "minecraft:shield"
                for slot in state.equipped_items.values()
            ),
            "inventory": state.inventory_agg,
            "nearest_hostile": (
                nearest_mob["name"]
                if nearest_mob and nearest_mob.get("is_hostile")
                else None
            ),
            "nearest_hostile_dist": (
                nearest_mob.get("distance")
                if nearest_mob and nearest_mob.get("is_hostile")
                else None
            ),
            "is_raining": (
                state.env_state.get("is_raining") if state.env_state else None
            ),
        }
    ...
    def _build_user_prompt(self, state_summary: Dict[str, Any]) -> str:
        allowed = "\n".join(
            [f"- {k}: {v['description']}" for k, v in self.allowed_actions.items()]
        )
        return (
    ...
    def _action_to_subgoal(
        self, action: str, params: Dict[str, Any], priority: int
    ) -> Optional[Subgoal]:
        if action in ("none", "hold", "wait", "noop"):
            return None

        mapping = {
            "emergency_retreat": Subgoal(
                name="emergency_retreat", params=params, priority=priority
            ),
